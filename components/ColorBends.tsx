...
void main() {
  ...
  if (uTransparent > 0) {
      // Transparent Mode: Output pure wave color and let CSS handle background
      finalCol = clamp(sumCol, 0.0, 1.0);
      finalAlpha = clamp(cover, 0.0, 1.0);
      finalCol *= finalAlpha; // Premultiply for standard blending
  } else {
      // Opaque Mode: Manual Composition
      // "Source Over" Blending: Result = Source + Dest * (1 - SourceAlpha)
      // Source is 'sumCol', Dest is 'uBgColor', SourceAlpha is 'cover'
      
      float invAlpha = 1.0 - clamp(cover, 0.0, 1.0);
      finalCol = sumCol + uBgColor * invAlpha;
...
export default function ColorBends({
  ...
  speed = 0.2,
  colors = [],
  transparent = true, // Note: If false, canvas is opaque and uses backgroundColor
  autoRotate = 0,
  scale = 1,
  frequency = 1,
  warpStrength = 1,
  ...
  // Helper to convert hex to Three.js Vector3
  ...
  useEffect(() => {
    ...
    // We clear to 0 alpha so the shader logic handles the "Background" or "Transparency"
  ...
  useEffect(() => {
    ...
    (material.uniforms.uBgColor.value as THREE.Vector3).copy(bgVec);
