...
export interface ScrollCarouselProps {
  ...
  direction?: 'left' | 'right'; // Direction of scroll - left or right
}
...
const useFeatureAnimations = (
  ...
  useLayoutEffect(() => {
    let ctx = gsap.context(() => {
      ...
      if (isDesktop) {
        ...
        const startX1 = direction === 'left' ? viewportOffset : -finalOffset1 + viewportOffset;
        const endX1 = direction === 'left' ? -finalOffset1 + viewportOffset : viewportOffset;
        
        const startX2 = direction === 'left' ? -finalOffset2 + viewportOffset * 2 : viewportOffset;
        const endX2 = direction === 'left' ? viewportOffset : -finalOffset2 + viewportOffset * 2;
        ...
        gsap
          ...
          .to(scrollContainerRef2.current, { x: endX2, ease: "none" });
  ...
  }, [isDesktop, maxScrollHeight, direction]);
};

// --- Component Definition ---
export const ScrollCarousel = forwardRef<HTMLDivElement, ScrollCarouselProps>(
  ({ features, className, maxScrollHeight, direction }, ref) => {
    ...
    return (
      ...
      >
        <div
          ref={containerRef}
          className="relative overflow-hidden md:h-[50vh] md:py-10
